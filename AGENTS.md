...
The workspace targets stable Rust (pinned via `rust-toolchain.toml`). `cargo build` automatically compiles the Node.js frontend: the `codex-plus-manager` build script runs `npm install` (when `node_modules` is absent) and `npm run vite:build` before Tauri compiles, so no manual frontend step is required. On release builds, the `codex-plus-packager` crate copies whatever release artifacts are present — launcher binary, manager binary, and manager cdylib (`codex_plus_manager_lib.dll`/`.dylib`/`.so`) — into `dist/<platform>/app/` and bundles them into `dist/<platform>/CodexPlusPlus-<version>-<platform>-<arch>.zip`. It does NOT use cargo artifact dependencies (bindeps): the manager library build dependency only guarantees the cdylib is compiled before the packager runs, while the binaries may still be linking. `cargo:rerun-if-changed` directives on each artifact ensure a subsequent `cargo build --release` re-runs the packager and picks up any binary that was not ready on the first pass — run `cargo build --release` twice if the zip is missing a binary.
